      ...
      fallback={
        <>
          ...
          <main className="mx-auto max-w-4xl px-4 py-10 md:py-16">
            <div className="text-sm text-muted">Loading report form…</div>
          </main>
          <SiteFooter />
        </>
      }
...
function ReportLostFoundForm() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      foundDisposition:
        reportType === "found"
          ? String(f.get("foundDisposition") || "seen_not_secured")
          : null,
      publicPhotoConsent,
      publicLocationConsent,
    };
    if (!publicLocationConsent) {
      return setError("Confirm that the approximate public location can be shown before publishing.");
    }
  ...
  return (
    <>
      <SiteHeader />
      <main className="mx-auto max-w-4xl px-4 py-10 md:py-16">
        <Link href="/lost-and-found" className="text-sm font-semibold text-primary hover:underline">
          ← Lost &amp; Found
        </Link>
        <div className="mt-5">
          ...
          <p className="mt-3 max-w-2xl text-muted">
            Only the information shown in this report is intended for public display. PawProof
            account information and private pet records stay private.
          </p>
        </div>
        ...
        <form onSubmit={submit} className="mt-8 space-y-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <button
                ...
                className={`rounded-xl border p-4 text-left ${
                  reportType === "lost" ? "border-primary bg-primary-soft" : "border-border"
                }`}
              >
                <div className="font-bold">My pet is missing</div>
                <div className="mt-1 text-sm text-muted">Create a missing-pet report.</div>
              </button>
              <button
                type="button"
                onClick={() => setReportType("found")}
                className={`rounded-xl border p-4 text-left ${
                  reportType === "found" ? "border-primary bg-primary-soft" : "border-border"
                }`}
              >
                <div className="font-bold">I found or saw a pet</div>
                <div className="mt-1 text-sm text-muted">No owned-pet profile is required.</div>
              </button>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-surface p-6">
            ...
            <label className="mt-4 block text-sm font-medium">
              ...
              <textarea
                name="approachInstructions"
                rows={3}
                placeholder="Shy around strangers, responds to treats…"
                className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-2"
              />
            </label>
            <div className="mt-4 rounded-xl border border-dashed border-border p-4 text-sm text-muted">
              Photo upload processing will use PawProof&apos;s private-to-public image pipeline.
              Until that pipeline is enabled, reports can be published without a photo rather
              than exposing an original upload directly.
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-surface p-6">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <Field
                label={reportType === "lost" ? "Last seen date/time" : "Found/seen date/time"}
                name="eventAt"
                type="datetime-local"
              ...
              <Field
                ...
                required
                placeholder="Near Speedway Blvd & Country Club Rd"
              />
            ...
            {reportType === "found" ? (
              <label className="mt-4 block text-sm font-medium">
                ...
                <select
                  ...
                  defaultValue="seen_not_secured"
                  className="mt-1 h-11 w-full rounded-xl border border-border bg-background px-3"
                >
                  <option value="in_care">In my care</option>
                  <option value="seen_not_secured">Seen but not secured</option>
                </select>
              </label>
            ) : null}
          </section>
          ...
          <section className="rounded-2xl border border-border bg-surface p-6">
            ...
            <p className="mt-2 text-sm text-muted">
              Public pages can be shared without exposing your PawProof account email, account
              ID, private documents, or microchip numbers.
            </p>
            <label className="mt-4 flex gap-3 text-sm">
              <input
                type="checkbox"
                checked={publicLocationConsent}
                onChange={(e) => setPublicLocationConsent(e.target.checked)}
                className="mt-1"
              />
              <span>I understand the approximate location and report details will be public.</span>
            </label>
            <label className="mt-3 flex gap-3 text-sm">
              <input
                type="checkbox"
                checked={publicPhotoConsent}
                onChange={(e) => setPublicPhotoConsent(e.target.checked)}
                className="mt-1"
              />
              <span>I allow selected report photos to be public after image processing is enabled.</span>
            </label>
          </section>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <Button type="submit" size="lg" disabled={submitting || loading}>
              {submitting ? "Publishing…" : user ? "Publish free report" : "Sign in and continue"}
            </Button>
